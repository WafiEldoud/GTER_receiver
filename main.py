...
class User(db.Model):
    ...
    approvalstatus = db.Column(db.Boolean, default=0) 
...
class UserAdminView(ModelView):
    column_list = ('username', 'email', 'person_code', 'approvalstatus')
    column_searchable_list = ('username', 'email')
    column_filters = ('username', 'email', 'approvalstatus')
...
def register():
    ...
    if request.method == 'POST':
        ...
        cur2.execute("SELECT * FROM public.user WHERE username = %s", (user_name,))
        result_1 = cur2.fetchone()
        cur3.execute("SELECT * FROM public.user WHERE email = %s", (user_email,))
        result_2 = cur3.fetchone()
        cur4.execute("SELECT * FROM public.user WHERE person_code = %s", (user_pc,))
        ...
        if result_2:
            ...
            cur.execute("INSERT INTO public.user"
...
def input():
    if request.method == 'POST':
        ...
        cur_L = mydb.cursor()
        cur_L.execute('SELECT username FROM public.student_data')
        ...
        if user in names:
            ...
            cur = mydb.cursor()
            cur.execute("INSERT INTO public.student_data"
...
def dataa():
    ...
    query = 'SELECT * FROM public.user WHERE username = %s LIMIT 1;'
    cursor.execute(query, (user,))  # Fixed here
    ...
    if user_id == None:
        ...
        displayed = [user_id[1], user_id[2], user_id[3]]
        cursor.close()
...
# Display members' data for nonmembers
...
def nonmembers():
    ...
    data =[(i[1], i[2], i[3]) for i in user_id]
    cursor.close()
...
def send_email():
    if request.method == 'POST':
        if session.get('loggedin') and session.get('username') == base['coordinator']:
            ...
            approved_users = User.query.filter_by(approvalstatus=True).all()
...
def new_members():
    if request.method == 'POST':
        if session.get('loggedin') and session.get('username') == base['coordinator']:
            subject = request.form['subject']
            body = request.form['body']
            approved_users = User.query.filter_by(approvalstatus=False).all()
            for user in approved_users:
                msg = Message(subject=subject, recipients=[user.email])
                msg.body = body
                mail.send(msg)
